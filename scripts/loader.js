...
((Const.CODE_PROGRESSBAR =
  'g.drawRect(10,g.getHeight()-16,g.getWidth()-10,g.getHeight()-8).flip();p=x=>g.fillRect(10,g.getHeight()-16,10+(g.getWidth()-20)*x/100,g.getHeight()-8);'),
  (Const.NO_RESET = true));
...
function onFoundDeviceInfo(deviceId, deviceVersion) {
  ...
  if (deviceId != 'PIPBOY') {
    showToast(
      `You're using ${deviceId}, not a Pip-Boy! Did you want <a href="https://espruino.com/apps">espruino.com/apps</a> instead?`,
      'warning',
      20000,
    );
    ...
    showToast(
      `You're using an old firmware (${deviceVersion}) and ${RECOMMENDED_VERSION} is available. <a href="https://thewand.co/pip-boy/upgrade/?file=fwupdate_${RECOMMENDED_VERSION}.zip">Click here to update</a>`,
      'warning',
      20000,
    );
  }
