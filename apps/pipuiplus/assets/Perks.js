...
class PerkEntry extends Entry {
  ...
  get description() {
    return this.data.d[Math.max(0, this.data.r - 1)];
  }
  ...
  drawStars(bounds) {
    ...
    for (let i = 0; i < starCount; ++i) {
      ...
      starImage.buffer = filled
        ? PerkEntry.FILLED_STAR_BUFFER
        : PerkEntry.HOLLOW_STAR_BUFFER;
      bC.drawImage(
        starImage,
        starsX + i * (starImage.width + starGap),
        starsY,
        { scale: 1 },
      );
    }
...
class PerkEditorEntry extends PerkEntry {
  constructor(data, index, array, indices) {
    // TODO: weird that we pass so much stuff in. Would be nice to shorten this
    super(data);
    this.index = index;
    this.array = array;
...
class IndexedFileBackedArray extends FileBackedArray {
  ...
  } // TODO: weird that we need the null check here, getter shouldn't be called anywhere when constructing the object
...
const enabledPerkIndices = fs
  .readFile(ENABLED_PERKS_PATH)
  .split(',')
  .map((s) => parseInt(s));
const perksArray = new FileBackedArray(
  E.openFile(PERKS_PATH),
  E.openFile(PERKS_PATH, 'w+'),
  74,
  701,
).lazyMap((o, i, a) => new PerkEditorEntry(o, i, a, enabledPerkIndices));
const enabledPerksArray = new IndexedFileBackedArray(
  perksArray,
  enabledPerkIndices,
