...
const copy = (original) =>
  Object.assign(Object.create(Object.getPrototypeOf(original)), original); // shallow copy preserving prototype
...
class FileBackedArray {
  ...
  slice(start, end) {
    ...
    start =
      ...
      end >= 0 ? Math.min(end, this.length) : Math.max(end + this.length, 0);

    const slice = [];
    for (let i = start; i < end; ++i) slice.push(this.get(i));
    return slice;
  }
...
class CachingWindow {
  ...
  moveTo(index) {
    ...
    if (delta > 0) {
      ...
      this.values.push.apply(
        this.values,
        this.array.slice(newEndIndex - delta, newEndIndex),
      );
    } else if (delta < 0) {
      this.values.splice(delta);
      this.values.unshift.apply(
        this.values,
        this.array.slice(newStartIndex, newStartIndex - delta),
      );
    }
...
class Menu {
  ...
  constructor(options, rows) {
    ...
    this.options = options = Object.assign(
      { x1: 10, x2: -20, y1: 0, y2: -1, titlePadX: 20 },
      options,
    );
    ...
    this.visibleRows = new CachingWindow(
      rows,
      Math.min(
        (options.y2 - options.y1 - Menu.ARROW_SIZE * 2) / options.rowHeight,
        rows.length,
      ) | 0,
    ); // | 0 to quickly floor
  ...
  get selectedRow() {
    return this.visibleRows.values[
      this.selectedIndex - this.visibleRows.startIndex
    ];
  }
  ...
  draw() {
    ...
    this.visibleRows.values.forEach((row, visibleRowIndex) => {
      const isSelected =
        firstVisibleRowIndex + visibleRowIndex == this.selectedIndex;
      ...
      isEditing && // draw edit box
        bC
          .setBgColor(3)
          .clearRect(
            valueX - bC.stringWidth(rowValue) - valuePadX * 2,
            rowY1,
            x2,
            rowY2,
          ) // draw highlight
          ...
          .drawImage(
            arrowsImage,
            valueX,
            rowY1 + ((rowHeight - arrowsImage.height * 2) >> 1),
            { scale: 2 },
          ); // draw up/down arrows
    ...
    bC.setColor(lastVisibleRowIndex < this.rows.length ? 3 : 0)
      .fillPoly([cx - arrowSize, by, cx + arrowSize, by, cx, by + arrowSize])
      .flip();
  }
  ...
  click() {
    ...
    if (valueType == 'n')
      this.isEditing = !this.isEditing; // number
    else {
...
class Entry {
  ...
  clear(bounds) {
    bC.reset().clearRect(bounds.left, bounds.top, bounds.right, bC.getHeight());
  }

  drawImage(bounds) {
    const imageX = (bounds.left + (bounds.right - 10) - this.data.w) >> 1; // centered horizontally (but bounds are padded 10 from the right)
    const imageY =
      bounds.top + ((bounds.bottom - bounds.top - this.data.h) * 3) / 4; // 3/4ths vertically
  ...
  drawDescription(bounds) {
    bC.setFont('Vector', 12).drawString(
      bC
        .wrapString(this.description, bounds.right - 10 - (bounds.left + 10))
        .join('\n'),
      bounds.left + 10,
      bounds.top,
    ); // padded 10 left and right
  }
