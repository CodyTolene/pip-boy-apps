...
const AST = [
  // asteroid polygon as X/Y pairs
  0, -1.5, 1, 0, 0.5, 0, 0.5, 0.5, 0, 1, -1, 0, -1, -1,
];
const SHIP = [SS, 0, -0.6 * SS, 0.4 * SS, -0.3 * SS, 0, -0.6 * SS, -0.4 * SS];
...
function onFrame() {
  ...
  timeSinceFired += d;
  if (KNOB1_BTN.read() && timeSinceFired > 4) {
    // fire!
    timeSinceFired = 0;
    ammo.push({
  ...
  ammo.forEach(function (a) {
    ...
    if (!hit && a.x >= 0 && a.y >= 0 && a.x < W && a.y < H) na.push(a);
  });
  ammo = na;
  na = [];
  var crashed = false;
  ast.forEach(function (a) {
    ...
    G.drawPolyAA(
      g.transformVertices(AST, { x: a.x, y: a.y, scale: a.rad, rotate: t }),
      true,
    );
    if (a.x < 0) a.x += W;
    if (a.y < 0) a.y += H;
