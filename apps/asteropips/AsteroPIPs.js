...
if (!g.drawPoly)
  g.drawPoly = function (p) {
    ...
    for (var i = 0; i < p.length; i += 2) g.lineTo(p[i], p[i + 1]);
  };
...
function onFrame() {
  ...
  framesSinceFired++;
  if (BTNA.read() && framesSinceFired > 5) {
    // fire!
    framesSinceFired = 0;
    ammo.push({
...
function initStart() {
  g.clear();
  Pip.typeText('Welcome to AsteroPIPs!').then(() =>
    setTimeout(() => {
      Pip.typeText(
        'Right Tune UP/DOWN = Rotate\nRight Tune PUSH = Forward\nLeft PUSH = Start / Fire!\nTop Torch = Exit',
      ).then(() => {
