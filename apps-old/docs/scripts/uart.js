...
(function (root, factory) {
  ...
  function parseRJSON(str) {
    let lex = (function (str) {
      ...
      var allowedRegExChars = [
        ...
        ';',
        ':',
      ]; // based on Espruino jslex.c (may not match spec 100%)
      var ch;
      var idx = 0;
      var lineNumber = 1;
      var nextCh = function () {
        ...
        if (isIn(chAlpha, ch)) {
          ...
          if (
            ...
            (lastToken.type == 'STRING' ||
              ...
              (lastToken.type == 'ID' &&
                !allowedRegExIDs.includes(lastToken.str)) ||
              (lastToken.type == 'CHAR' &&
                !allowedRegExChars.includes(lastToken.str)))
          ) {
            // https://www-archive.mozilla.org/js/language/js20-2000-07/rationale/syntax.html#regular-expressions
            type = 'CHAR';
          } else {
    ...
    function match(s) {
      if (tok.str != s)
        throw new Error('Expecting ' + s + ' got ' + JSON.stringify(tok.str));
      tok = lex.next();
    }
  ...
  function handleQueue() {
    ...
    if (q.type == 'eval') uart.eval(q.expr, q.cb).then(q.resolve, q.reject);
      ...
      uart
        .write(q.data, q.callback, q.callbackNewline)
  ...
  class Connection {
    ...
    on(evt, cb) {
      ...
      if (!this[e]) this[e] = [];
      this[e].push(cb);
    } // on only works with a single handler
    emit(evt, data1, data2) {
      let e = 'on' + evt;
      if (this[e]) this[e].forEach((fn) => fn(data1, data2));
    }
    removeListener(evt, callback) {
      let e = 'on' + evt;
      if (this[e]) this[e] = this[e].filter((fn) => fn != callback);
    }
      ...
      if (this.parsePackets) {
        for (var i = 0; i < data.length; i++) {
          ...
          if (this.rxDataHandlerPacket !== undefined) {
            ...
            ch = undefined;
            let flags =
              (this.rxDataHandlerPacket.charCodeAt(0) << 8) |
              this.rxDataHandlerPacket.charCodeAt(1);
            ...
            if (rxLen >= 2 && rxLen >= len + 2) {
              ...
              this.emit(
                'packet',
                flags & 0xe000,
                this.rxDataHandlerPacket.substring(2),
              );
              this.rxDataHandlerPacket = undefined; // stop packet reception
            }
          } else if (ch == '\x06') {
            // handle individual control chars
            log(3, 'Got ACK');
            this.emit('ack');
            ch = undefined;
          } else if (ch == '\x15') {
            ...
            // DLE - potential start of packet (ignore)
            this.rxDataHandlerLastCh = '\x10';
            ch = undefined;
          } else if (ch == '\x01' && this.rxDataHandlerLastCh == '\x10') {
            // SOH
          ...
          if (ch === undefined) {
            // if we're supposed to remove the char, do it
            data = data.substring(0, i) + data.substring(i + 1);
            i--;
          } else this.rxDataHandlerLastCh = ch;
        }
      }
      this.hadData = true;
      if (data.length > 0) {
        // keep track of received data
        if (this.received.length < 100000)
          // ensure we're not creating a memory leak
          this.received += data;
        // forward any data
  ...
  */
    ...
    espruinoSendFile(filename, data, options) {
      ...
      let connection = this;
      let packetCount = 0,
        packetTotal = Math.ceil(data.length / CHUNK) + 1;
      // always ack the FILE_SEND
      progressHandler(0, packetTotal);
      return connection
        .espruinoSendPacket('FILE_SEND', JSON.stringify(options))
        .then(sendData);
      ...
      function sendData() {
        ...
        return connection
          .espruinoSendPacket('DATA', packet, packetOptions)
          .then(sendData);
      }
    }
    ...
    espruinoEval(expr, options) {
      ...
      return new Promise((resolve, reject) => {
        ...
        connection
          .espruinoSendPacket('EVAL', expr, { noACK: options.stmFix })
          .then(
            () => {
  ...
  endpoints.push({
     ...
     */
            ...
            txCharacteristic
              ...
              .then(function () {
                ...
                if (!txItem.data) {
                  ...
                  if (txItem.resolve) txItem.resolve();
                }
      ...
      return navigator.bluetooth
        .requestDevice(uart.optionsBluetooth)
     ...
     */
      ...
      connection.write = function (data, callback, alreadyRetried) {
        ...
        return new Promise((resolve, reject) => {
          ...
          if (serialPort.writable.locked) {
            if (alreadyRetried) return reject('Writable stream is locked');
            log(0, 'Writable stream is locked - retry in 500ms');
            setTimeout(() => {
              this.write(data, callback, true).then(resolve, reject);
            }, 500);
            return;
          }
      ...
      return (
        options.serialPort
          ...
          : navigator.serial.requestPort(uart.optionsSerial)
      )
        .then(function (port) {
          ...
          function readLoop() {
            ...
            reader.read().then(
              function ({ value, done }) {
                reader.releaseLock();
                reader = undefined;
                if (value) connection.rxDataHandler(ab2str(value.buffer));
                if (done) {
...
*/
  function createModal(options) {
    ...
    menu.style =
      'position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);font-family: Sans-Serif;z-index:101;min-width:300px';
    ...
    items.style =
      'color:#000;background:#fff;padding:4px 8px 4px 8px;min-height:4em';
    if ('string' == typeof options.contents) items.innerHTML = options.contents;
    else items.appendChild(options.contents);
    menu.appendChild(items);
    document.body.appendChild(e);
    document.body.appendChild(menu);
    e.onclick = function (evt) {
      // clicked modal -> remove
      evt.preventDefault();
      result.remove();
      if (options.onClickBackground) options.onClickBackground();
    };
    menu.onclick = function (evt) {
      // clicked menu
      evt.preventDefault();
      if (options.onClickMenu) options.onClickMenu();
    };
  ...
  function evaluate(expr, cb) {
    if (!checkIfSupported()) return;
    if (isBusy) return pushToQueue({ type: 'eval', expr: expr, cb: cb });
    return write(
